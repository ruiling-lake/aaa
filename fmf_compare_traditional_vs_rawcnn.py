...
1) Traditional baseline:
   per-temp Linear MIMO/FIR

2) Proposed:
   Raw center-tap + temp-aware 1D-CNN residual equalizer
   + joint loss (regression + classification)
...
# =========================
# 5) Traditional baseline: per-temp Linear MIMO/FIR
# =========================
@dataclass
...
def build_loader(dataset: Dataset, batch_size: int, shuffle: bool):
    return DataLoader(dataset, batch_size=int(batch_size), shuffle=shuffle, num_workers=0, drop_last=False)
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default=r"E:\pycharm file\fiberproject\fiberdsp\runs_compare_traditional_vs_rawcnn")
    ...
    y_linear_train = predict_per_temp_linear(X_feat_train, tg_train, linear_models, linear_global_model)
    y_linear_val = predict_per_temp_linear(X_feat_val, tg_val, linear_models, linear_global_model)
    y_linear_test = predict_per_temp_linear(X_feat_test, tg_test, linear_models, linear_global_model)

    z_linear_test = complex_from_iq(y_linear_test)
    traditional_metrics_test = compute_metrics(z_linear_test, y_test, class_constellation, class_bits)
    traditional_by_temp_test = compute_metrics_by_temp(z_linear_test, y_test, tg_test, class_constellation, class_bits)

    # -------- 6) Proposed: Raw center-tap + temp-aware 1D-CNN residual --------
    ...
    proposed_by_temp_test = compute_metrics_by_temp(z_nn_test, y_test, tg_test, class_constellation, class_bits)
    ...
    traditional_lowtemp_test = compute_metrics_on_mask(
        z_linear_test, y_test, t_test,
        class_constellation, class_bits,
        temp_min=args.lowtemp_min, temp_max=args.lowtemp_max
    )
    ...
    metrics_pack = {
        ...
        "traditional_test": {k: v for k, v in traditional_metrics_test.items() if k != "y_pred_idx"},
        "proposed_test": {k: v for k, v in proposed_metrics_test.items() if k != "y_pred_idx"},
        "raw_by_temp_test": raw_by_temp_test,
        "traditional_by_temp_test": traditional_by_temp_test,
        "proposed_by_temp_test": proposed_by_temp_test,
        "raw_lowtemp_test": raw_lowtemp_test,
        "traditional_lowtemp_test": traditional_lowtemp_test,
        "proposed_lowtemp_test": proposed_lowtemp_test,
    }

    save_json(metrics_pack, os.path.join(args.save_dir, "metrics_compare_traditional_vs_rawcnn.json"))

    # -------- 9) 绘图 --------
    plot_training_curve(
        out_tempaware.history,
        os.path.join(args.save_dir, "training_curve_proposed.png"),
        "Raw center-tap + Temp-aware 1D-CNN Residual Training Curve",
    )
    ...
    plot_constellation(
        z_linear_test,
        class_constellation[y_test],
        "After Traditional Baseline (Per-temp Linear MIMO-FIR)",
        os.path.join(args.save_dir, "constellation_after_traditional.png"),
    )

    plot_constellation(
        z_nn_test,
        class_constellation[y_test],
        "After Proposed Method (Raw center-tap + Temp-aware 1D-CNN Residual)",
        os.path.join(args.save_dir, "constellation_after_proposed.png"),
    )
    ...
    plot_constellation_lowtemp(
        ...
        class_constellation[y_test],
        t_test,
        f"Low-temperature Raw Center-tap ({args.lowtemp_min:.0f}C ~ {args.lowtemp_max:.0f}C)",
        os.path.join(args.save_dir, "constellation_lowtemp_raw.png"),
        temp_min=args.lowtemp_min,
        temp_max=args.lowtemp_max,
    )
    ...
    plot_constellation_lowtemp(
        ...
        class_constellation[y_test],
        t_test,
        f"Low-temperature Proposed ({args.lowtemp_min:.0f}C ~ {args.lowtemp_max:.0f}C)",
        os.path.join(args.save_dir, "constellation_lowtemp_proposed.png"),
        temp_min=args.lowtemp_min,
        temp_max=args.lowtemp_max,
    )
    ...
    acc_curves = {
        ...
        "Traditional baseline accuracy": traditional_by_temp_test,
        "Proposed method accuracy": proposed_by_temp_test,
    }
    ...
    plot_lowtemp_bar(
        {
            "Raw": raw_lowtemp_test,
            "Traditional": traditional_lowtemp_test,
            "Proposed": proposed_lowtemp_test,
        },
    ...
    print("  Traditional baseline (Per-temp Linear MIMO/FIR):")
    print(f"    Accuracy = {metrics_pack['traditional_test']['accuracy']:.6f}")
    print(f"    SER      = {metrics_pack['traditional_test']['SER']:.6f}")
    print(f"    BER      = {metrics_pack['traditional_test']['BER']:.6f}")
    print(f"    EVM      = {metrics_pack['traditional_test']['EVM']:.6f}")

    print("  Proposed method (Raw center-tap + Temp-aware 1D-CNN Residual):")
    print(f"    Accuracy = {metrics_pack['proposed_test']['accuracy']:.6f}")
    print(f"    SER      = {metrics_pack['proposed_test']['SER']:.6f}")
    print(f"    BER      = {metrics_pack['proposed_test']['BER']:.6f}")
    print(f"    EVM      = {metrics_pack['proposed_test']['EVM']:.6f}")

    print(f"\n低温段 ({args.lowtemp_min:.0f}C ~ {args.lowtemp_max:.0f}C) 指标：")
    print("  Raw center-tap baseline:")
    ...
    print("  Traditional baseline (Per-temp Linear MIMO/FIR):")
    print(f"    Accuracy = {metrics_pack['traditional_lowtemp_test']['accuracy']:.6f}")
    print(f"    SER      = {metrics_pack['traditional_lowtemp_test']['SER']:.6f}")
    print(f"    BER      = {metrics_pack['traditional_lowtemp_test']['BER']:.6f}")
    print(f"    EVM      = {metrics_pack['traditional_lowtemp_test']['EVM']:.6f}")

    print("  Proposed method (Raw center-tap + Temp-aware 1D-CNN Residual):")
    print(f"    Accuracy = {metrics_pack['proposed_lowtemp_test']['accuracy']:.6f}")
    print(f"    SER      = {metrics_pack['proposed_lowtemp_test']['SER']:.6f}")
    print(f"    BER      = {metrics_pack['proposed_lowtemp_test']['BER']:.6f}")
    print(f"    EVM      = {metrics_pack['proposed_lowtemp_test']['EVM']:.6f}")
